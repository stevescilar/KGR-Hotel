...
@section('breadcrumb', 'Room management')
    ...
    <div class="bg-white rounded-xl border border-gray-100 overflow-hidden">
        <div class="flex items-center gap-1 p-4 border-b border-gray-100 overflow-x-auto">
            <button onclick="filterType('all')" id="tab-all"
                    class="tab-btn px-4 py-2 rounded-lg text-sm font-medium whitespace-nowrap bg-[#1e3a2f] text-white transition-colors">
                All Rooms
            </button>
            @foreach($roomTypes as $type)
            <button onclick="filterType('{{ $type->id }}')" id="tab-{{ $type->id }}"
                    class="tab-btn px-4 py-2 rounded-lg text-sm font-medium whitespace-nowrap text-gray-500 hover:bg-gray-100 transition-colors">
        ...
        <div class="p-5 grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4" id="roomsGrid">
            ...
            @endphp
            <div class="room-card border-2 rounded-xl p-4 {{ $statusStyle }}" data-type="{{ $room->room_type_id }}">
                <div class="flex items-start justify-between mb-3">
                    <div>
                        ...
                        <div class="text-xs text-gray-500 ml-4.5">{{ $room->roomType->name }}</div>
                    </div>
                    <div class="text-right">
                        <div class="text-xs text-gray-400">{{ $room->floor ? 'Floor ' . $room->floor : ($room->cottage ? 'Cottage' : '') }}</div>
                        <div class="text-xs font-semibold text-gray-600 capitalize">{{ str_replace('_', ' ', $room->status) }}</div>
                    </div>
                </div>

                {{-- Occupied: show current guest --}}
                ...
                <div class="bg-white/60 rounded-lg p-2.5 mb-3 text-xs">
                    <div class="font-semibold text-gray-700">{{ $activeBooking->guest->full_name }}</div>
                    <div class="text-gray-400">Until {{ $activeBooking->check_out->format('M j') }}</div>
                </div>
                @endif
                @endif
                ...
                <form method="POST" action="{{ route('admin.rooms.status', $room) }}" class="flex items-center gap-2">
                    ...
                    <button type="submit" class="bg-[#1e3a2f] text-white px-3 py-1.5 rounded-lg text-xs font-semibold hover:bg-[#2e5c42] transition-colors">
                        Update
                    </button>
                </form>
            </div>
...
function filterType(typeId) {
    ...
    document.querySelectorAll('.tab-btn').forEach(btn => {
        btn.classList.toggle('bg-[#1e3a2f]', btn.id === 'tab-' + typeId);
        btn.classList.toggle('text-white', btn.id === 'tab-' + typeId);
        btn.classList.toggle('text-gray-500', btn.id !== 'tab-' + typeId);
    });
}
</script>
@endpush
