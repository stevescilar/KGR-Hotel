...
<div class="p-6 max-w-xl mx-auto">
    <div class="mb-6">
        ...
        <h1 class="text-2xl font-display text-gray-900 mt-1">Add Menu Category</h1>
    </div>

    <form method="POST" action="{{ route('admin.restaurant.menu.categories.store') }}" enctype="multipart/form-data"
          class="bg-white rounded-xl shadow-sm border border-gray-100 p-6 space-y-5">
        @csrf

        <div>
            <label class="block text-xs font-bold text-green-700 uppercase tracking-widest mb-1">Category Name *</label>
            <input type="text" name="name" value="{{ old('name') }}" required
                   placeholder="e.g. Breakfast, Grills, Cocktails, Desserts"
        ...
        <div>
            <label class="block text-xs font-bold text-green-700 uppercase tracking-widest mb-2">Category Image</label>
            <div class="border-2 border-dashed border-gray-200 rounded-xl p-6 text-center cursor-pointer hover:border-green-400 transition-colors"
                ...
                <img id="imgPreview" src="" alt="" class="mx-auto mb-3 rounded-lg max-h-32 object-cover hidden">
                <div id="imgPrompt">
                    <div class="text-3xl mb-2">🍴</div>
                    <div class="text-sm text-gray-500">Click to upload category image</div>
